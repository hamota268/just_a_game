...
SECOND ENTER THE FILE PATH OF THE MUSIC YOU WANT, THEN:
		...
		type 'clear' to clean the screen,
		type 'exit' to exit the story mode. {emojies.SMILE}""")		
		while True:	
			story_command = input(CYAN + (">>> ").lower())
			#story_mode_starter
			if story_command == "start":
				music_path_command = input(YELLOW + ("ENTER SONG PATH(press 'enter' key if you don't):>> "))
				timer()
				#music_player
				while True:
					try:
						pyg.mixer.init()
						pyg.mixer.music.load(music_path_command)
						sleep(3)
